...
class InitialBuilder:
    ...
    def build_reference_ass(
        self, ids_dict: dict[str, list[str]], cog_dict: dict[str, list[str]], uni_dict: dict[str, str], amb_dict: dict[str, str], char_range: range
    ):
        ...
        for codepoint in char_range:
            ...
            for ids in ids_dict[char]:
                for cognition in cog_dict[char]:
                    if (set(_sub(ids, amb_dict)).issubset(set(_sort(cognition))) or set(_sub(ids, amb_dict)) == set(char)) and re.sub(
                        ids_filter, "", ids
                    ) != "":
...
if __name__ == "__main__":
    InitialBuilder(0x20000, 0x200FF)
