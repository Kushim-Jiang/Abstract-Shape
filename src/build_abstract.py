...
def parse_ass_dict_from_yamls():
    ass_paths = ["data/abstract_shape_main.yaml", "data/abstract_shape_a.yaml", "data/abstract_shape_comp.yaml", "data/abstract_shape_b.yaml"]
    parsed_ass_dict = {}
    for ass_path in ass_paths:
        with open(ass_path, 'r', encoding="utf-8") as f_ass:
            ass_dict = yaml.load(f_ass, Loader=yaml.FullLoader)
        for key, val in ass_dict.items():
            parsed_val = parse_ass(val)
            parsed_ass_dict[key] = parsed_val

    # dump yaml
    temp_ass_path = "res_abstract/ass.yaml"
    left_brace = ["", "["]
    right_brace = ["", "]"]
...
def parse_ass_dict(ass_dict: dict[str, list[str]]) -> dict[str, list[str]]:
    while True:
    ...
    # dump yaml
    temp_ass_path = "res_abstract/iterative_ass.yaml"
    left_brace = ["", "["]
    right_brace = ["", "]"]
...
def build_as_dict(ass_dict: dict[str, list[str]]) -> dict[int, str]:
    as_set = set()
    ...
    # dump yaml
    temp_ass_path = "res_abstract/as_dict.yaml"
    with open(temp_ass_path, 'w', encoding="utf-8") as f_temp:
...
def build_indexed_ass_dict(ass_dict: dict[str, list[str]], as_dict: dict[int, str]) -> dict[str, list[int]]:
    ...
    # dump yaml
    temp_ass_path = "res_abstract/indexed_ass.yaml"
    with open(temp_ass_path, 'w', encoding="utf-8") as f_temp:
        yaml.dump(indexed_dict, f_temp, indent=4, allow_unicode=True)
...
def build_unification() -> None:
    ass_path = "res_abstract/iterative_ass.yaml"
    ...
    temp_unification_path = "res_abstract/unification.txt"
...
def main():
    parsed_ass_dict = parse_ass_dict_from_yamls()
    iteratively_parsed_ass_dict = parse_ass_dict(parsed_ass_dict)
    as_dict = build_as_dict(iteratively_parsed_ass_dict)
    parsed_indexed_list = build_indexed_ass_dict(iteratively_parsed_ass_dict, as_dict)
    build_unification()
